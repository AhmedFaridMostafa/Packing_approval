...
export async function GET(request: Request) {
  ...
  try {
    ...
    const region_id = searchParams.has("region_id")
      ? parseInt(searchParams.get("region_id")!)
      : undefined;
    const country_id = searchParams.has("country_id")
      ? parseInt(searchParams.get("country_id")!)
      : undefined;
    const category_id = searchParams.has("category_id")
    ...
    const data = await getPackingHistory({
      ...
      action:
        action && ["CREATE", "UPDATE", "DELETE"].includes(action)
          ? action
          : undefined,
